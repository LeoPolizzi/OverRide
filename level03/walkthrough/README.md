When looking at the disassembled code here, we see some interesting things:
...
Dump of assembler code for function main:
   ...
   0x080488ca <+112>:   movl   $0x1337d00d,0x4(%esp) <==== Absolute value passed to test
   0x080488d2 <+120>:   mov    %eax,(%esp)
   0x080488d5 <+123>:   call   0x8048747 <test> <==== Call to test function with user input as first param, absolute value as second param
...
Dump of assembler code for function test:
   ...
   0x08048755 <+14>:    sub    %eax,%ecx <==== Subtract user input from absolute value
...
Bunch of random checks that all do the same final thing, which is to call decrypt with the value of hardcoded value - user input value.

Dump of assembler code for function decrypt:                                                                                                                16:16:31 [40/1038]
   ...
   0x08048673 <+19>:    movl   $0x757c7d51,-0x1d(%ebp) <==== Hardcoded obfuscated password
   0x0804867a <+26>:    movl   $0x67667360,-0x19(%ebp) <==== Hardcoded obfuscated password
   0x08048681 <+33>:    movl   $0x7b66737e,-0x15(%ebp) <==== Hardcoded obfuscated password
   0x08048688 <+40>:    movl   $0x33617c7d,-0x11(%ebp) <==== Hardcoded obfuscated password
   0x0804868f <+47>:    movb   $0x0,-0xd(%ebp) <==== Null terminator for the string
   ...
   0x080486d5 <+117>:   xor    %edx,%eax <==== Simple xor encryption with the value from the function parameter
   ...
   0x080486f2 <+146>:   mov    $0x80489c3,%eax <==== Address of the string "Congratulations!" used for comparison
   0x080486f7 <+151>:   mov    $0x11,%ecx
   0x080486fc <+156>:   mov    %edx,%esi
   0x080486fe <+158>:   mov    %eax,%edi
   0x08048700 <+160>:   repz cmpsb %es:(%edi),%ds:(%esi) <==== Compare the decrypted string with "Congratulations!"
   ...
   0x08048715 <+181>:   movl   $0x80489d4,(%esp) <==== String "/bin/sh" is passed to system
   0x0804871c <+188>:   call   0x80484e0 <system@plt> <==== Call to system with "/bin/sh" if the decrypted string matches "Congratulations!"
...
So all I have to do is to do is find the correct value for the decryption and then find the input that can get converted to this value.

I made a program in C that will give me the correct value for decryption:
...
This then prints the correct value which is:

i: 18 = 0x12

So now that I know the value used for decryption, I only need to find the correct input that gets converted to this value.

Earlier I saw that there was a substraction on an hardcoded value with my input:

0x1337d00d - X = 18

I only need to find X now.

0x1337d00d = 322424845 in decimal

Substract 18 to get 322424827 which will be our input.
